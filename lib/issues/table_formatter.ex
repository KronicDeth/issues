defmodule Issues.TableFormatter do
  ...
  headers.  Prints a table to STDOUT of the data from each row identified by
  each header.  That is, each header identifies a column, and those columns are
  extracted and printed from the rows.  We calculate the width of each column
  ...
  and a format string, write the extracted data under control for the format
  string.
  """
  ...
  @doc """
  Generate the line that goes below the column headings.  Is is a string of
  ...
  ## Example
      ...
      ...>          [ {"a", "4"}, {"b", "5"}, {"c", "6"} ] ]
      iex> list_of_hash_dicts = Issues.CLI.convert_to_list_of_hash_dicts(list)
      iex> Issues.TableFormatter.split_into_columns(list_of_hash_dicts, ["a", "b", "c"])
      [ ["1", "4"], ["2", "5"], ["3", "6"] ]
  """
  ...
  column, return a list containing the maximum widht of each column.
